...
class MockStripeCustomer:
    def __init__(self, id="cus_test123", email="test@example.com", name="Test User"):
        self.id = id
        self.email = email
        self.name = name

class MockStripeCheckoutSession:
    def __init__(self, id="cs_test123", url="https://checkout.stripe.com/test", **kwargs):
        self.id = id
        self.url = url
        for key, value in kwargs.items():
            setattr(self, key, value)
...
class MockStripeEvent:
    def __init__(self, type="checkout.session.completed", data=None):
        self.type = type
...
async def setup_test_db_integration(db_session):
    ...
    try:
        ...
        await session.execute(text("DELETE FROM transactions WHERE organization_id IN (SELECT id FROM organizations WHERE name LIKE 'Test Billing%')"))
    ...
    try:
        ...
        async for cleanup_session in db_session:
            try:
                await cleanup_session.execute(text("DELETE FROM transactions WHERE organization_id IN (SELECT id FROM organizations WHERE name LIKE 'Test Billing%')"))
                await cleanup_session.execute(text("DELETE FROM wallets WHERE organization_id IN (SELECT id FROM organizations WHERE name LIKE 'Test Billing%')"))
