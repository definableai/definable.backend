...
def mock_knowledge_base():
  ...
  return MockKnowledgeBaseModel(
    ...
    settings={"embedding_model": "openai", "max_chunk_size": 1000, "chunk_overlap": 100, "separator": "\n\n", "version": 1},
  )
...
def mock_kb_document():
  ...
  return MockKBDocumentModel(
    ...
    source_metadata={"file_type": "pdf", "original_filename": "test.pdf", "size": 1024, "mime_type": "application/pdf"},
  )
...
def mock_kb_service():
  ...
  async def mock_get(org_id, kb_id, session, user):
    ...
    kb = MockKnowledgeBaseModel(id=kb_id, organization_id=org_id, name="Test Knowledge Base", collection_id=uuid4())

    # Create some documents
    documents = [MockKBDocumentModel(kb_id=kb_id, title=f"Document {i}", s3_key=f"uploads/doc{i}.pdf") for i in range(1, 3)]
  ...
  async def mock_list(org_id, session, user):
    ...
    kbs = [MockKnowledgeBaseModel(organization_id=org_id, name=f"Knowledge Base {i}") for i in range(1, 3)]
  ...
  async def mock_add_file_document(org_id, kb_id, document_data, background_tasks, session, user):
    ...
    await s3_client_mock.upload_file(file=BytesIO(file_content), key=s3_key)
    ...
    background_tasks.add_task(kb_service._process_document_task, MockSourceTypeModel.FILE, doc, session)

    # Generate download URL for response
    download_url = await s3_client_mock.get_presigned_url(s3_key, expires_in=3600, operation="get_object")
  ...
  async def mock_add_url_document(org_id, kb_id, document_data, background_tasks, session, user):
    ...
    # Add document processing task to background tasks
    background_tasks.add_task(kb_service._process_document_task, MockSourceTypeModel.URL, doc, session)
  ...
  async def mock_get_document_chunks(org_id, kb_id, doc_id, limit, offset, session, user):
    ...
    total_chunks = 5
    chunks = [
      {"id": uuid4(), "chunk_id": i, "content": f"Chunk {i} content", "metadata": {"source": doc.title, "chunk": i}}
    ...
    return MockResponse(
      document_id=doc_id, title=doc.title, chunks=[MockResponse.model_validate(chunk) for chunk in chunks], total_chunks=total_chunks
    )
  ...
  async def mock__get_kb(kb_id, org_id, session):
    ...
    kb = MockKnowledgeBaseModel(id=kb_id, organization_id=org_id)
  ...
  async def mock__get_document(doc_id, org_id, session):
    ...
    doc = MockKBDocumentModel(id=doc_id)

    # Get the KB for this document
    kb = MockKnowledgeBaseModel(id=doc.kb_id, organization_id=org_id)
...
class TestKBService:
  ...
  async def test_create_knowledge_base(self, mock_kb_service, mock_db_session, mock_user):
    ...
    # Call the service
    response = await mock_kb_service.post_create(org_id, kb_data, session=mock_db_session, user=mock_user)
  ...
  async def test_update_knowledge_base(self, mock_kb_service, mock_db_session, mock_user):
    ...
    org_id = uuid4()
    kb_id = uuid4()
    update_data = MockResponse(name="Updated Knowledge Base")

    # Call the service
    response = await mock_kb_service.put_update(org_id, kb_id, update_data, session=mock_db_session, user=mock_user)
  ...
  async def test_get_knowledge_base(self, mock_kb_service, mock_db_session, mock_user):
    ...
    # Call the service
    response = await mock_kb_service.get_get(org_id, kb_id, session=mock_db_session, user=mock_user)
  ...
  async def test_get_knowledge_base_not_found(self, mock_kb_service, mock_db_session, mock_user):
    ...
    with pytest.raises(HTTPException) as excinfo:
      await mock_kb_service.get_get(org_id, kb_id, session=mock_db_session, user=mock_user)
  ...
  async def test_list_knowledge_bases(self, mock_kb_service, mock_db_session, mock_user):
    ...
    # Call the service
    response = await mock_kb_service.get_list(org_id, session=mock_db_session, user=mock_user)
  ...
  async def test_delete_knowledge_base(self, mock_kb_service, mock_db_session, mock_user):
    ...
    # Call the service
    response = await mock_kb_service.delete_remove(org_id, kb_id, session=mock_db_session, user=mock_user)
  ...
  async def test_add_file_document(self, mock_kb_service, mock_db_session, mock_user, mock_upload_file, mock_background_tasks):
    ...
    response = await mock_kb_service.post_add_file_document(
      org_id, kb_id, document_data, mock_background_tasks, session=mock_db_session, user=mock_user
    )
  ...
  async def test_add_url_document(self, mock_kb_service, mock_db_session, mock_user, mock_background_tasks):
    ...
    document_data = MockResponse(
      ...
      operation="scrape",
      source_id=None,
      settings={"excludeTags": [""], "includeTags": [""], "onlyMainContent": True, "formats": ["text"]},
      source_metadata={
        "url": "https://example.com",
        "operation": "scrape",
        "settings": {"excludeTags": [""], "includeTags": [""], "onlyMainContent": True, "formats": ["text"]},
  ...
  async def test_get_document_chunks(self, mock_kb_service, mock_db_session, mock_user):
    ...
    # Call the service
    response = await mock_kb_service.get_get_document_chunks(org_id, kb_id, doc_id, limit, offset, session=mock_db_session, user=mock_user)
  ...
  async def test_remove_document(self, mock_kb_service, mock_db_session, mock_user):
    ...
    # Call the service
    response = await mock_kb_service.delete_remove_document(org_id, doc_id, session=mock_db_session, user=mock_user)
  ...
  async def test_search_chunks(self, mock_kb_service, mock_db_session, mock_user):
    ...
    # Call the service
    response = await mock_kb_service.post_search_chunks(org_id, kb_id, query, limit, score_threshold, session=mock_db_session, user=mock_user)
  ...
  async def test_update_knowledge_base_not_found(self, mock_kb_service, mock_db_session, mock_user):
    ...
    # Verify exception is raised
    with pytest.raises(HTTPException) as excinfo:
      await mock_kb_service.put_update(org_id, kb_id, update_data, session=mock_db_session, user=mock_user)
  ...
  async def test_delete_knowledge_base_not_found(self, mock_kb_service, mock_db_session, mock_user):
    ...
    # Verify exception is raised
    with pytest.raises(HTTPException) as excinfo:
      await mock_kb_service.delete_remove(org_id, kb_id, session=mock_db_session, user=mock_user)
  ...
  async def test_add_file_document_kb_not_found(self, mock_kb_service, mock_db_session, mock_user, mock_upload_file, mock_background_tasks):
    ...
    # Verify exception is raised
    with pytest.raises(HTTPException) as excinfo:
      await mock_kb_service.post_add_file_document(org_id, kb_id, document_data, mock_background_tasks, session=mock_db_session, user=mock_user)
  ...
  async def test_add_url_document_kb_not_found(self, mock_kb_service, mock_db_session, mock_user, mock_background_tasks):
    ...
    # Verify exception is raised
    with pytest.raises(HTTPException) as excinfo:
      await mock_kb_service.post_add_url_document(org_id, kb_id, document_data, mock_background_tasks, session=mock_db_session, user=mock_user)
  ...
  async def test_get_document_chunks_document_not_found(self, mock_kb_service, mock_db_session, mock_user):
    ...
    # Verify exception is raised
    with pytest.raises(HTTPException) as excinfo:
      await mock_kb_service.get_get_document_chunks(org_id, kb_id, doc_id, limit, offset, session=mock_db_session, user=mock_user)
  ...
  async def test_remove_document_not_found(self, mock_kb_service, mock_db_session, mock_user):
    ...
    # Verify exception is raised
    with pytest.raises(HTTPException) as excinfo:
      await mock_kb_service.delete_remove_document(org_id, doc_id, session=mock_db_session, user=mock_user)
  ...
  async def test_search_chunks_kb_not_found(self, mock_kb_service, mock_db_session, mock_user):
    ...
    # Verify exception is raised
    with pytest.raises(HTTPException) as excinfo:
      await mock_kb_service.post_search_chunks(org_id, kb_id, query, limit, score_threshold, session=mock_db_session, user=mock_user)
  ...
  async def test_add_url_document_with_crawl_operation(self, mock_kb_service, mock_db_session, mock_user, mock_background_tasks):
    ...
    document_data = MockResponse(
      ...
      settings={
        ...
        "scrapeOptions": {"excludeTags": ["nav", "footer"], "includeTags": [], "onlyMainContent": True, "formats": ["text", "html"]},
      },
      source_metadata={
        "url": "https://example.com",
        "operation": "crawl",
    ...
    async def mock_add_url_document_crawl(org_id, kb_id, document_data, background_tasks, session, user):
      ...
      # Add document processing task to background tasks
      background_tasks.add_task(mock_kb_service._process_document_task, MockSourceTypeModel.URL, doc, session)
  ...
  async def test_update_document_chunk(self, mock_kb_service, mock_db_session, mock_user):
    ...
    org_id = uuid4()
    kb_id = uuid4()
    doc_chunk_data = MockResponse(chunk_id=123, content="Updated chunk content")
    ...
    response = await mock_kb_service.put_update_document_chunk(
      org_id=org_id, kb_id=kb_id, doc_chunk_data=doc_chunk_data, session=mock_db_session, user=mock_user
    )
  ...
  async def test_update_document_chunk_not_found(self, mock_kb_service, mock_db_session, mock_user):
    ...
    org_id = uuid4()
    kb_id = uuid4()
    doc_chunk_data = MockResponse(chunk_id=999, content="Updated chunk content")
    ...
    # Verify exception is raised
    with pytest.raises(HTTPException) as excinfo:
      await mock_kb_service.put_update_document_chunk(org_id, kb_id, doc_chunk_data, session=mock_db_session, user=mock_user)
  ...
  async def test_delete_document_chunks(self, mock_kb_service, mock_db_session, mock_user):
    ...
    org_id = uuid4()
    kb_id = uuid4()
    doc_chunk_data = MockResponse(chunk_ids=["1", "2", "3"])
    ...
    # Call the service
    response = await mock_kb_service.delete_delete_document_chunks(org_id, kb_id, doc_chunk_data, session=mock_db_session, user=mock_user)
  ...
  async def test_get_document_content(self, mock_kb_service, mock_db_session, mock_user):
    ...
    # Call the service
    response = await mock_kb_service.get_get_document_content(org_id, doc_id, session=mock_db_session)
  ...
  async def test_index_documents(self, mock_kb_service, mock_db_session, mock_user, mock_background_tasks):
    ...
    async def mock_index_documents(org_id, kb_id, doc_ids, background_tasks, session, user):
      ...
      for doc_id in doc_ids:
        doc = MockKBDocumentModel(id=doc_id, kb_id=kb_id, indexing_status=DocumentStatus.PENDING)
        docs.append(doc)
      ...
      # Add background task
      background_tasks.add_task(mock_kb_service._indexing_documents_task, doc_ids, org_id, kb_id, session)
    ...
    # Call the service
    response = await mock_kb_service.post_index_documents(org_id, kb_id, doc_ids, mock_background_tasks, session=mock_db_session, user=mock_user)
  ...
  async def test_add_document_chunk(self, mock_kb_service, mock_db_session, mock_user):
    ...
    kb_id = uuid4()
    doc_id = uuid4()
    chunk_data = MockResponse(content="This is a new document chunk.")
    ...
    async def mock_add_document_chunk(org_id, kb_id, doc_id, chunk_data, session, user):
      ...
      # Create chunk object
      chunk = MockResponse(id=uuid4(), chunk_id=new_chunk_id, content=chunk_data.content, metadata={"source": doc.title, "chunk": new_chunk_id})
    ...
    # Call the service
    response = await mock_kb_service.post_add_document_chunk(org_id, kb_id, doc_id, chunk_data, session=mock_db_session, user=mock_user)
