...
from src.services.auth.schema import InviteSignup, StytchUser, TestLogin, TestResponse, TestSignup
...
def mock_request():
  ...
  request = MagicMock()
  request.headers = {"svix-id": "test-id", "svix-timestamp": "test-timestamp", "svix-signature": "test-signature"}
...
class TestAuthService:
  ...
  async def test_post_webhook_create_user(self, auth_service, mock_db_session, mock_request):
    ...
    with patch("src.services.auth.service.verify_svix_signature", return_value=True):
      ...
      with patch.object(auth_service, "_create_new_user", AsyncMock(return_value=mock_user)):
        ...
        with patch("src.services.auth.service.stytch_base.update_user", AsyncMock(return_value=LibResponse(success=True, data=update_response_data))):
  ...
  async def test_post_signup_invite(self, auth_service, mock_db_session):
    ...
    invite_data = InviteSignup(first_name="John", last_name="Doe", email="john.doe@example.com")
  ...
  async def test_post_test_signup_success(self, auth_service, mock_db_session, test_email):
    ...
    test_signup = TestSignup(first_name="Test", last_name="User", email=test_email, password="Password123*(^$!*(^&@*(&!")

    # Mock database user that will be created from the test signup
    mock_user = MockUserModel(id=uuid4(), email=test_email)
  ...
  async def test_post_test_login_success(self, auth_service, mock_db_session, test_email):
    ...
    signup_data = TestSignup(first_name="Test", last_name="User", email=test_email, password="Password123*(^$!*(^&@*(&!")
    ...
    test_login = TestLogin(email=test_email, password="Password123*(^$!*(^&@*(&!")
  ...
  async def test_create_new_user_success(self, auth_service, mock_db_session):
    ...
    stytch_user = StytchUser(email="john.doe@example.com", stytch_id="user-test-123", first_name="John", last_name="Doe", metadata={})
  ...
  async def test_create_new_user_existing_user(self, auth_service, mock_db_session):
    ...
    existing_user = MockUserModel(email="john.doe@example.com", stytch_id="user-test-123")
  ...
  async def test_setup_default_organization(self, auth_service, mock_db_session):
    ...
    # Create test role
    owner_role = MockRoleModel(id=uuid4(), name="owner", hierarchy_level=100)
