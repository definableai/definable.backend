...
from src.services.roles.schema import PermissionCreate, RoleCreate, RoleResponse, RoleUpdate
...
def mock_system_role():
  ...
  role_id = uuid4()
  return create_model_mock(
    id=role_id, name="OWNER", description="Owner Role", is_system_role=True, hierarchy_level=100, organization_id=uuid4(), permissions=[]
  )
...
def mock_permission():
  ...
  perm_id = uuid4()
  return create_model_mock(id=perm_id, name="test:read", resource="test", action="read", description="Test Permission")
...
class TestRoleService:
  ...
  async def test_delete_permission(self, roles_service, mock_db_session):
    ...
    await roles_service.delete_permission(permission_id=permission_id, session=mock_db_session)
  ...
  async def test_post_create_role_exists(self, roles_service, mock_db_session, test_user, mock_role):
    ...
    with pytest.raises(Exception) as exc_info:
      await roles_service.post_create(org_id=org_id, role_data=role_data, session=mock_db_session, user=test_user)
  ...
  async def test_put_update_success(self, roles_service, mock_db_session, test_user, mock_role):
    ...
    with (
      patch.object(roles_service, "_get_role", AsyncMock(return_value=mock_role)),
      ...
      # The test case
      role_data = RoleUpdate(name="Updated Role", description="Updated description", hierarchy_level=60)

      result = await roles_service.put_update(org_id=org_id, role_id=role_id, role_data=role_data, session=mock_db_session, user=test_user)
  ...
  async def test_put_update_not_found(self, roles_service, mock_db_session, test_user):
    ...
    with patch.object(roles_service, "_get_role", AsyncMock(return_value=None)):
      # The test case
      role_data = RoleUpdate(name="Updated Role", description="Updated description")

      # Expect an exception
      with pytest.raises(Exception) as exc_info:
        await roles_service.put_update(org_id=org_id, role_id=role_id, role_data=role_data, session=mock_db_session, user=test_user)
  ...
  async def test_put_update_system_role(self, roles_service, mock_db_session, test_user, mock_system_role):
    ...
    with patch.object(roles_service, "_get_role", AsyncMock(return_value=mock_system_role)):
      # The test case
      role_data = RoleUpdate(name="Updated System Role", description="Updated description")
  ...
  async def test_delete_remove_success(self, roles_service, mock_db_session, test_user, mock_role):
    ...
    # The test case
    result = await roles_service.delete_remove(org_id=org_id, role_id=role_id, session=mock_db_session, user=test_user)
  ...
  async def test_delete_remove_not_found(self, roles_service, mock_db_session, test_user):
    ...
    # Expect an exception
    with pytest.raises(Exception) as exc_info:
      await roles_service.delete_remove(org_id=org_id, role_id=role_id, session=mock_db_session, user=test_user)
  ...
  async def test_get_list_roles(self, roles_service, mock_db_session, test_user, mock_role, mock_system_role):
    ...
    # The test case
    result = await roles_service.get_list_roles(org_id=org_id, session=mock_db_session, user=test_user)
  ...
  async def test_get_list_permissions(self, roles_service, mock_db_session, test_user, mock_permission):
    ...
    # The test case
    result = await roles_service.get_list_permissions(session=mock_db_session, user=test_user)
...
class TestPermissionManagement:
  ...
  async def test_add_permissions_to_role(self, roles_service, mock_db_session, mock_role, mock_permission):
    ...
    with patch.object(roles_service, "_add_role_permissions", AsyncMock()) as mock_add_permissions:
      # Execute
      await roles_service._add_role_permissions(role_id=role_id, permission_ids=[permission_id], session=mock_db_session)

      # Assert - use keyword arguments to match how the method is called
      mock_add_permissions.assert_called_once_with(role_id=role_id, permission_ids=[permission_id], session=mock_db_session)
  ...
  async def test_get_role_with_permissions(self, roles_service, mock_db_session, mock_role, mock_permission):
    ...
    with patch("src.services.roles.schema.RoleResponse", return_value=MagicMock()):
      # Execute
      result = await roles_service._get_role_with_permissions(role_id=role_id, session=mock_db_session)
...
class TestRoleServiceErrorHandling:
  ...
  async def test_invalid_permission_ids(self, roles_service, mock_db_session, test_user):
    ...
    with patch.object(roles_service, "_validate_hierarchy_level", AsyncMock()):
      ...
      role_data = RoleCreate(
        name="Test Role", description="Role with invalid permissions", hierarchy_level=50, permission_ids=[invalid_permission_id]
      )
  ...
  async def test_hierarchy_level_constraints(self, roles_service, mock_db_session, test_user):
    ...
    role_data_high = RoleCreate(name="High Level Role", description="Role with too high hierarchy", hierarchy_level=95, permission_ids=[uuid4()])

    # Execute and Assert for high hierarchy
    with pytest.raises(HTTPException) as exc_info:
      await roles_service._validate_hierarchy_level(organization_id=org_id, hierarchy_level=role_data_high.hierarchy_level, session=mock_db_session)
  ...
  async def test_db_transaction_rollback(self, roles_service, mock_db_session, test_user, monkeypatch):
    ...
    org_id = uuid4()
    role_data = RoleCreate(
      name="Rollback Test Role", description="This role creation should trigger a rollback", hierarchy_level=50, permission_ids=[uuid4()]
    )
    ...
    with patch.object(roles_service, "_validate_hierarchy_level", AsyncMock()), patch.object(roles_service, "_add_role_permissions", AsyncMock()):
      # Execute and Assert
      with pytest.raises(Exception):
        await roles_service.post_create(org_id=org_id, role_data=role_data, session=mock_db_session, user=test_user)
...
class TestRoleServicePerformance:
  ...
  async def test_bulk_role_creation(self, roles_service, mock_db_session, test_organization):
    ...
    with (
      patch.object(roles_service, "_validate_hierarchy_level", AsyncMock()),
      ...
      for i in range(num_roles):
        role_data = RoleCreate(name=f"Bulk Role {i}", description=f"Bulk created role {i}", hierarchy_level=10 + i, permission_ids=[uuid4()])

        tasks.append(roles_service.post_create(org_id=org_id, role_data=role_data, session=mock_db_session, user=test_user))
...
class TestRoleServiceIntegration:
    ...
    async for session in db_session:
      try:
        ...
        permission_data = PermissionCreate(
          name="test:integration:read", resource="test:integration", action="read", description="Test integration read permission"
        )

        # Execute
        response = await roles_service.post_permission(permission=permission_data, session=session)
  ...
  async def test_create_role_integration(self, roles_service, db_session, integration_test_user, integration_test_org, setup_test_db_integration):
    ...
    async for session in db_session:
      try:
        ...
        permission_data = PermissionCreate(
          name="test:integration:write", resource="test:integration", action="write", description="Test integration write permission"
        )

        permission_response = await roles_service.post_permission(permission=permission_data, session=session)

        # Now create a role with that permission
        role_data = RoleCreate(
          name="Test Integration Role", description="Role created for integration test", hierarchy_level=50, permission_ids=[permission_response.id]
        )

        # Execute
        response = await roles_service.post_create(org_id=integration_test_org, role_data=role_data, session=session, user=integration_test_user)
  ...
  async def test_authorization_non_admin_create_role(self, roles_service, db_session, setup_test_db_integration):
    ...
    async for session in db_session:
      try:
        ...
        permission_data = PermissionCreate(
          name="test:integration:auth:read", resource="test:integration:auth", action="read", description="Test integration auth permission"
        )

        permission = await roles_service.post_permission(permission=permission_data, session=session)

        # Create role data with the real permission
        role_data = RoleCreate(
          name="Test Integration Unauthorized Role", description="Role created without permission", hierarchy_level=10, permission_ids=[permission.id]
        )

        # In a real integration test, the RBAC middleware would reject this
        # Here we're simulating that by mocking the dependency
        with patch("dependencies.security.RBAC", side_effect=HTTPException(status_code=403, detail="Insufficient permissions")):
          # Execute and Assert
          with pytest.raises(HTTPException) as exc_info:
            await roles_service.post_create(org_id=org_id, role_data=role_data, session=session, user=non_admin_user)
  ...
  async def test_authorization_update_higher_role(
    self, roles_service, db_session, integration_test_user, integration_test_org, setup_test_db_integration
  ):
    ...
    async for session in db_session:
      try:
        ...
        permission_data = PermissionCreate(
          name="test:integration:auth:write", resource="test:integration:auth", action="write", description="Test integration auth write permission"
        )
        ...
        high_role_response = await roles_service.post_create(
          org_id=integration_test_org, role_data=high_role_data, session=session, user=integration_test_user
        )
        ...
        # Create update data
        update_data = RoleUpdate(name="Updated High Role", description="Attempt to update higher role")

        # In a real integration test, the RBAC middleware would reject this
        # Here we're simulating that by mocking the dependency
        with patch(
          "dependencies.security.RBAC", side_effect=HTTPException(status_code=403, detail="Cannot modify a role with higher hierarchy level")
        ):
          # Execute and Assert
          with pytest.raises(HTTPException) as exc_info:
            await roles_service.put_update(
              org_id=integration_test_org, role_id=high_role_response.id, role_data=update_data, session=session, user=lower_hierarchy_user
            )
