...
def mock_roles_service():
    ...
    class RolesService:
        ...
        async def get_list_roles(self, organization_id, session, user):
            roles = [
                MockRoleModel(organization_id=organization_id, name="Role 1"),
                MockRoleModel(organization_id=organization_id, name="Role 2")
            ]
        ...
        async def get_list_permissions(self, session, user):
            permissions = [
                MockPermissionModel(name="Permission 1"),
                MockPermissionModel(name="Permission 2")
            ]
    ...
    for method_name in [
        'post_permission', 'delete_permission', 'post_create', 'put_update',
        'delete_remove', 'get_list_roles', 'get_list_permissions',
        '_get_role', '_get_role_with_permissions'
    ]:
...
class TestRoleService:
    ...
    async def test_delete_permission(self, mock_roles_service, mock_db_session):
        ...
        # Call the service
        await mock_roles_service.delete_permission(
            uuid4(),
            mock_db_session
        )
    ...
    async def test_create_role(self, mock_roles_service, mock_db_session, mock_user):
        ...
        # Call the service
        response = await mock_roles_service.post_create(
            org_id,
            role_data,
            mock_db_session,
            mock_user
    ...
    async def test_create_role_invalid_hierarchy(self, mock_roles_service, mock_db_session, mock_user):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await mock_roles_service.post_create(
                org_id,
                role_data,
                mock_db_session,
                mock_user
    ...
    async def test_update_role(self, mock_roles_service, mock_db_session, mock_user):
        ...
        # Call the service
        response = await mock_roles_service.put_update(
            org_id,
            role_id,
            update_data,
            mock_db_session,
    ...
    async def test_update_nonexistent_role(self, mock_roles_service, mock_db_session, mock_user):
        ...
        # Verify exception is raised
        with pytest.raises(HTTPException) as exc_info:
            await mock_roles_service.put_update(
                org_id,
                role_id,
                update_data,
                mock_db_session,
    ...
    async def test_delete_role(self, mock_roles_service, mock_db_session, mock_user):
        ...
        # Call the service
        response = await mock_roles_service.delete_remove(
            uuid4(),
            uuid4(),
            mock_db_session,
            mock_user
    ...
    async def test_get_roles_list(self, mock_roles_service, mock_db_session, mock_user):
        ...
        # Call the service
        response = await mock_roles_service.get_list_roles(
            uuid4(),
            mock_db_session,
            mock_user
        )
    ...
    async def test_get_permissions_list(self, mock_roles_service, mock_db_session, mock_user):
        ...
        # Call the service
        response = await mock_roles_service.get_list_permissions(
            mock_db_session,
            mock_user
        )
    ...
    async def test_create_role_duplicate_name(self, mock_roles_service, mock_db_session, mock_user):
        ...
        role_data = MockResponse(
            name="Existing Role",
            description="Role with duplicate name",
            hierarchy_level=10,
            permission_ids=[uuid4()]
        )
        
        # Set up mock to simulate existing role with same name
        existing_role = MockRoleModel(
            id=uuid4(),
            name="Existing Role",
            organization_id=org_id
        )
    ...
    async def test_delete_system_role(self, mock_roles_service, mock_db_session, mock_user):
        ...
        # Verify exception is raised
        with pytest.raises(HTTPException) as exc_info:
            await mock_roles_service.delete_remove(
                org_id,
                role_id,
                mock_db_session,
                mock_user
    ...
    async def test_get_role(self, mock_roles_service, mock_db_session, mock_user):
        ...
        # Call the service
        response = await mock_roles_service.get_role(
            org_id,
            role_id,
            mock_db_session,
            mock_user
    ...
    async def test_get_role_not_found(self, mock_roles_service, mock_db_session, mock_user):
        ...
        # Verify exception is raised
        with pytest.raises(HTTPException) as exc_info:
            await mock_roles_service.get_role(
                org_id,
                role_id,
                mock_db_session,
                mock_user
    ...
    async def test_create_permission_duplicate(self, mock_roles_service, mock_db_session):
        ...
        permission_data = MockResponse(
            ...
            resource="roles",
            action="read",
            description="Permission with duplicate resource/action"
        )
        ...
        async def mock_create_permission_duplicate(permission, session):
            ...
            existing_permission = MockPermissionModel(
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Permission for '{permission.resource}:{permission.action}' already exists"
            )
        ...
        # Verify exception is raised
        with pytest.raises(HTTPException) as exc_info:
            await mock_roles_service.post_permission(
                permission_data,
                mock_db_session
            )
