...
class TestPromptService:
  ...
  async def test_get_list_categories_active_only(self, prompt_service, mock_db_session, mock_user, mock_categories):
    ...
    # Execute
    response = await prompt_service.get_list_categories(active_only=True, session=mock_db_session, user=mock_user)
  ...
  async def test_get_list_categories_all(self, prompt_service, mock_db_session, mock_user, mock_categories):
    ...
    # Execute
    response = await prompt_service.get_list_categories(active_only=False, session=mock_db_session, user=mock_user)
  ...
  async def test_post_create_category_success(self, prompt_service, mock_db_session, mock_user):
    ...
    category_data = PromptCategoryCreate(
      name="New Category", description="A new category description", icon_url="https://example.com/new-icon.png", display_order=10
    )
    ...
    new_category = MockPromptCategoryModel(
      name=category_data.name, description=category_data.description, icon_url=category_data.icon_url, display_order=category_data.display_order
    )
    ...
    with patch("src.services.prompts.service.PromptCategoryModel", return_value=new_category):
      # Execute
      response = await prompt_service.post_create_category(category_data=category_data, session=mock_db_session, user=mock_user)
  ...
  async def test_post_create_category_duplicate(self, prompt_service, mock_db_session, mock_user, mock_category):
    ...
    # Setup
    category_data = PromptCategoryCreate(name=mock_category.name, description="A new category description")
    ...
    with pytest.raises(HTTPException) as exc_info:
      await prompt_service.post_create_category(category_data=category_data, session=mock_db_session, user=mock_user)
  ...
  async def test_put_update_category_success(self, prompt_service, mock_db_session, mock_user, mock_category):
    ...
    category_data = PromptCategoryUpdate(name="Updated Category Name", description="Updated description", display_order=20)
    ...
    # Execute
    await prompt_service.put_update_category(category_id=category_id, category_data=category_data, session=mock_db_session, user=mock_user)
  ...
  async def test_delete_delete_category_success(self, prompt_service, mock_db_session, mock_user, mock_category):
    ...
    response = await prompt_service.delete_delete_category(category_id=category_id, session=mock_db_session, user=mock_user)
  ...
  async def test_delete_delete_category_not_found(self, prompt_service, mock_db_session, mock_user):
    ...
    # Execute and assert
    with pytest.raises(HTTPException) as exc_info:
      await prompt_service.delete_delete_category(category_id=category_id, session=mock_db_session, user=mock_user)
  ...
  async def test_get_get_category_success(self, prompt_service, mock_db_session, mock_user, mock_category):
    ...
    # Execute
    response = await prompt_service.get_get_category(category_id=category_id, session=mock_db_session, user=mock_user)
  ...
  async def test_get_get_category_not_found(self, prompt_service, mock_db_session, mock_user):
    ...
    # Execute and Assert
    with pytest.raises(HTTPException) as exc_info:
      await prompt_service.get_get_category(category_id=category_id, session=mock_db_session, user=mock_user)
  ...
  async def test_get_list_prompts(self, prompt_service, mock_db_session, mock_user, mock_prompts):
    ...
    response = await prompt_service.get_list_prompts(org_id=org_id, session=mock_db_session, user=mock_user)
  ...
  async def test_get_list_prompts_with_category_filter(self, prompt_service, mock_db_session, mock_user, mock_prompts, mock_category):
    ...
    filtered_prompts = [p for p in mock_prompts if p.category_id == category_id and (p.organization_id == org_id or p.is_public)]
    ...
    mock_db_session.execute = AsyncMock(return_value=execute_mock)
    # Execute the actual service method
    response = await prompt_service.get_list_prompts(org_id=org_id, category_id=category_id, session=mock_db_session, user=mock_user)
  ...
  async def test_post_create_prompt_category_not_found(self, prompt_service, mock_db_session, mock_user):
    ...
    org_id = uuid4()
    category_id = uuid4()
    prompt_data = PromptCreate(title="New Prompt", content="This is a new prompt content")
    ...
    with pytest.raises(HTTPException) as exc_info:
      await prompt_service.post_create_prompt(
        org_id=org_id, category_id=category_id, prompt_data=prompt_data, session=mock_db_session, user=mock_user
      )
  ...
  async def test_put_update_prompt_not_found(self, prompt_service, mock_db_session, mock_user):
    ...
    prompt_data = PromptUpdate(title="Updated Prompt Title", content="Updated prompt content")
    ...
    with pytest.raises(HTTPException) as exc_info:
      await prompt_service.put_update_prompt(
        org_id=org_id, category_id=category_id, prompt_id=prompt_id, prompt_data=prompt_data, session=mock_db_session, user=mock_user
      )
  ...
  async def test_delete_delete_prompt_success(self, prompt_service, mock_db_session, mock_user, mock_prompt):
    ...
    # Execute
    response = await prompt_service.delete_delete_prompt(org_id=org_id, prompt_id=prompt_id, session=mock_db_session, user=mock_user)
  ...
  async def test_delete_delete_prompt_not_found(self, prompt_service, mock_db_session, mock_user):
    ...
    # Execute and assert
    with pytest.raises(HTTPException) as exc_info:
      await prompt_service.delete_delete_prompt(org_id=org_id, prompt_id=prompt_id, session=mock_db_session, user=mock_user)
  ...
  async def test_get_list_all_prompts(self, prompt_service, mock_db_session, mock_user, mock_prompts):
    ...
    with patch("sqlalchemy.select", MagicMock()), patch("sqlalchemy.orm.joinedload", MagicMock()):
      response = await prompt_service.get_list_all_prompts(session=mock_db_session, user=mock_user)
  ...
  async def test_update_prompt_all_fields(self, prompt_service, mock_db_session, mock_user, mock_prompt, mock_category):
    ...
    # Manually update the mock_prompt with the values from prompt_data
    mock_prompt.title = prompt_data.title
    mock_prompt.content = prompt_data.content
    mock_prompt.description = prompt_data.description
    mock_prompt.is_public = prompt_data.is_public
    mock_prompt.is_featured = prompt_data.is_featured
    mock_prompt.metadata = prompt_data.metadata
  ...
  async def test_prompt_update_metadata_merge(self, prompt_service, mock_db_session, mock_user, mock_prompt, mock_category):
    ...
    prompt_data = PromptUpdate(title="Updated Title", content="Updated Content", metadata={"tags": ["updated"], "new_field": "value"})
    ...
    assert mock_prompt.metadata == prompt_data.metadata
    assert not (mock_prompt.metadata and "count" in mock_prompt.metadata)
    assert isinstance(mock_prompt.metadata, dict)
    assert mock_prompt.metadata.get("tags") == ["updated"]
...
def test_integration_user():
  ...
  user_id = uuid4()
  org_id = uuid4()
  return {"id": user_id, "email": f"test-integration-{user_id}@example.com", "first_name": "Test", "last_name": "Integration", "org_id": org_id}
...
class TestPromptServiceIntegration:
  ...
  async def test_category_crud_integration(self, prompt_service, db_session, test_integration_user, setup_test_db_integration):
    ...
    async for session in db_session:
      try:
        # 1. Create category
        category_data = PromptCategoryCreate(
          name="Test Integration Category", description="Test integration description", icon_url="https://example.com/icon.png", display_order=1
        )

        response = await prompt_service.post_create_category(category_data=category_data, session=session, user=test_integration_user)
        ...
        get_response = await prompt_service.get_get_category(category_id=category_id, session=session, user=test_integration_user)

        assert get_response.id == category_id
        assert get_response.name == category_data.name

        # 3. Update the category
        update_data = PromptCategoryUpdate(name="Test Integration Category Updated", description="Updated description")

        update_response = await prompt_service.put_update_category(
          category_id=category_id, category_data=update_data, session=session, user=test_integration_user
        )
        ...
        delete_response = await prompt_service.delete_delete_category(category_id=category_id, session=session, user=test_integration_user)
        ...
        # Verify deletion
        with pytest.raises(HTTPException) as exc_info:
          await prompt_service.get_get_category(category_id=category_id, session=session, user=test_integration_user)
  ...
  async def test_prompt_crud_integration(self, prompt_service, db_session, test_integration_user, setup_test_db_integration):
    ...
    async for session in db_session:
      try:
        ...
        category_data = PromptCategoryCreate(name="Test Integration Prompt Category", description="Category for prompt testing")

        category = await prompt_service.post_create_category(category_data=category_data, session=session, user=test_integration_user)
        ...
        prompt_response = await prompt_service.post_create_prompt(
          org_id=test_integration_user["org_id"], category_id=category.id, prompt_data=prompt_data, session=session, user=test_integration_user
        )
        ...
        # 3. Update the prompt
        update_data = PromptUpdate(title="Test Integration Prompt Updated", content="Updated content for integration test", is_featured=True)
        ...
        delete_response = await prompt_service.delete_delete_prompt(
          org_id=test_integration_user["org_id"], prompt_id=prompt_id, session=session, user=test_integration_user
        )
        ...
        await prompt_service.delete_delete_category(category_id=category.id, session=session, user=test_integration_user)
  ...
  async def test_pagination_integration(self, prompt_service, db_session, test_integration_user, setup_test_db_integration):
    ...
    async for session in db_session:
      try:
        # 1. Create a category
        category_data = PromptCategoryCreate(name="Test Integration Pagination Category", description="Category for pagination testing")
        ...
        page_1 = await prompt_service.get_list_prompts(org_id=test_integration_user["org_id"], session=session, user=test_integration_user)
        ...
        # 4. Test second page
        page_2 = await prompt_service.get_list_prompts(org_id=test_integration_user["org_id"], offset=1, session=session, user=test_integration_user)
        ...
        small_page = await prompt_service.get_list_prompts(
          org_id=test_integration_user["org_id"], limit=5, session=session, user=test_integration_user
        )
        ...
        category_filter = await prompt_service.get_list_prompts(
          org_id=test_integration_user["org_id"], category_id=category.id, session=session, user=test_integration_user
        )
        ...
        featured_filter = await prompt_service.get_list_prompts(
          org_id=test_integration_user["org_id"], is_featured=True, session=session, user=test_integration_user
        )
        ...
        for prompt in page_1.prompts:
          await prompt_service.delete_delete_prompt(
            org_id=test_integration_user["org_id"], prompt_id=prompt.id, session=session, user=test_integration_user
          )
  ...
  async def test_public_prompts_integration(self, prompt_service, db_session, test_integration_user, setup_test_db_integration):
    ...
    async for session in db_session:
      try:
        # 1. Create a category
        category_data = PromptCategoryCreate(name="Test Integration Public Category", description="Category for public prompts testing")
        ...
        for i in range(3):
          ...
          await prompt_service.post_create_prompt(
            org_id=second_user["org_id"], category_id=category.id, prompt_data=prompt_data, session=session, user=second_user
          )

        # 4. Test that user 1 can see their prompts plus user 2's public prompts
        user1_prompts = await prompt_service.get_list_prompts(
          org_id=test_integration_user["org_id"], include_public=True, session=session, user=test_integration_user
        )
        ...
        user2_prompts = await prompt_service.get_list_prompts(org_id=second_user["org_id"], include_public=True, session=session, user=second_user)
        ...
        user1_private_only = await prompt_service.get_list_prompts(
          org_id=test_integration_user["org_id"], include_public=False, session=session, user=test_integration_user
        )
        ...
        # Cleanup prompts
        all_prompts = await prompt_service.get_list_pagination(org_id=test_integration_user["org_id"], session=session, user=test_integration_user)
  ...
  async def test_create_prompt_success_integration(self, prompt_service, db_session, test_integration_user, setup_test_db_integration):
    ...
    async for session in db_session:
      try:
        # 1. Create a category first
        category_data = PromptCategoryCreate(
          name="Test Integration Create Success Category", description="Category for create prompt success testing"
        )
        ...
        # Clean up
        await prompt_service.delete_delete_prompt(
          org_id=test_integration_user["org_id"], prompt_id=prompt_response.id, session=session, user=test_integration_user
        )
  ...
  async def test_create_prompt_minimal_fields_integration(self, prompt_service, db_session, test_integration_user, setup_test_db_integration):
    ...
    async for session in db_session:
      try:
        # 1. Create a category first
        category_data = PromptCategoryCreate(name="Test Integration Minimal Fields Category", description="Category for minimal fields testing")
  ...
  async def test_update_prompt_success_integration(self, prompt_service, db_session, test_integration_user, setup_test_db_integration):
    ...
    async for session in db_session:
      try:
        # 1. Create a category first
        category_data = PromptCategoryCreate(name="Test Integration Update Prompt Category", description="Category for update prompt testing")
        ...
        # Clean up
        await prompt_service.delete_delete_prompt(
          org_id=test_integration_user["org_id"], prompt_id=updated_prompt.id, session=session, user=test_integration_user
        )
