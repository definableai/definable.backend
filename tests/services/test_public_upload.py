...
def mock_public_upload_service():
  ...
  async def mock_post(file, session, user):
    ...
    # Upload to S3
    url = await s3_client.upload_file(file=BytesIO(file_content), key=f"uploads/{file.filename}")

    # Create DB record
    current_time = datetime.now().isoformat()
    db_upload = MockPublicUploadModel(filename=file.filename, content_type=file.content_type, url=url, created_at=current_time)
    session.add(db_upload)
    await session.commit()

    # Return response
    return MockResponse(id=db_upload.id, url=url, filename=file.filename, content_type=file.content_type, created_at=current_time)
...
class TestPublicUploadService:
  ...
  async def test_upload_file(self, mock_public_upload_service, mock_db_session, mock_user, mock_upload_file):
    ...
    # Call the service
    response = await mock_public_upload_service.post(file=mock_upload_file, session=mock_db_session, user=mock_user)
  ...
  async def test_upload_file_with_special_characters(self, mock_public_upload_service, mock_db_session, mock_user):
    ...
    # Call the service
    response = await mock_public_upload_service.post(file=file, session=mock_db_session, user=mock_user)
  ...
  async def test_upload_file_s3_exception(self, mock_public_upload_service, mock_db_session, mock_user, mock_upload_file):
    ...
    async def mock_post_with_exception(file, session, user):
      try:
        ...
        await mock_public_upload_service.s3_client.upload_file(file=BytesIO(file_content), key=f"uploads/{file.filename}")
        # The following code should not execute
        db_upload = MockPublicUploadModel(filename=file.filename, content_type=file.content_type, url="https://example.com/uploads/test.jpg")
        session.add(db_upload)
        await session.commit()
  ...
  async def test_upload_file_bad_request(self, mock_public_upload_service, mock_db_session, mock_user, mock_upload_file):
    ...
    with pytest.raises(HTTPException) as exc_info:
      await mock_public_upload_service.post(file=mock_upload_file, session=mock_db_session, user=mock_user)
