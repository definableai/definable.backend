...
class TestPublicUploadService:
  ...
  async def test_upload_file_s3_exception(self, public_upload_service, mock_db_session, mock_user, mock_upload_file):
    ...
    with pytest.raises(Exception) as exc_info:
      await public_upload_service.post(file=mock_upload_file, session=mock_db_session, user=mock_user)
  ...
  async def test_unsupported_file_type(self, public_upload_service, mock_db_session, mock_user):
    ...
    with pytest.raises(HTTPException) as exc_info:
      await post_with_validation(file=file, session=mock_db_session, user=mock_user)
  ...
  async def test_exceeds_size_limit(self, public_upload_service, mock_db_session, mock_user):
    ...
    # Call directly without mocking
    with pytest.raises(HTTPException) as exc_info:
      await post_with_validation(file=oversized_file, session=mock_db_session, user=mock_user)
  ...
  async def test_authorization_check(self, public_upload_service, mock_db_session, mock_user):
    ...
    async def post_with_auth_check(file, session, user, required_role="editor"):
      ...
      role_levels = {"admin": 100, "editor": 50, "viewer": 10}
      ...
      url = await public_upload_service.s3_client.upload_file(file=BytesIO(file_content), key=key)

      # Store in database
      db_upload = MockPublicUploadModel(filename=file.filename, content_type=file.content_type, url=url)
    ...
    with pytest.raises(HTTPException) as exc_info:
      await post_with_auth_check(file=file, session=mock_db_session, user=test_user, required_role="editor")
    ...
    response = await post_with_auth_check(file=file, session=mock_db_session, user=test_user, required_role="editor")
  ...
  async def test_metadata_handling(self, public_upload_service, mock_db_session, mock_user):
    ...
    async def post_with_metadata(file, metadata, session, user):
      ...
      db_upload = MockPublicUploadModel(filename=file.filename, content_type=file.content_type, url=url, _metadata=metadata)
      session.add(db_upload)
      await session.commit()
    ...
    # Define metadata
    metadata = {"author": "Test User", "description": "Test file with metadata", "tags": ["test", "metadata", "example"], "is_public": True}

    # Call the service
    response = await public_upload_service.post_with_metadata(file=file, metadata=metadata, session=mock_db_session, user=mock_user)
  ...
  async def test_file_content_validation(self, public_upload_service, mock_db_session, mock_user):
    ...
    response = await post_with_image_validation(file=valid_jpeg, session=mock_db_session, user=mock_user)
    ...
    # Test invalid image
    with pytest.raises(HTTPException) as exc_info:
      await post_with_image_validation(file=invalid_jpeg, session=mock_db_session, user=mock_user)
  ...
  async def test_custom_storage_path(self, public_upload_service, mock_db_session, mock_user):
    ...
    async def post_with_custom_path(file, session, user):
      ...
      url = await public_upload_service.s3_client.upload_file(file=BytesIO(file_content), key=custom_key)
    ...
    response = await post_with_custom_path(file=file, session=mock_db_session, user=mock_user)
  ...
  async def test_user_specific_uploads(self, public_upload_service, mock_db_session, mock_user):
    ...
    async def post_user_specific(file, session, user):
      ...
      db_upload = MockPublicUploadModel(filename=file.filename, content_type=file.content_type, url=url, _metadata={"user_id": str(user_id)})
      session.add(db_upload)
      await session.commit()
    ...
    # Call the method directly
    response = await post_user_specific(file=file, session=mock_db_session, user=mock_user)
...
def test_upload_file():
  ...
  try:
    ...
    upload_file = TestUploadFile(filename="test_integration_image.jpg", file=file_obj, content_type="image/jpeg")
    ...
    async for session in get_db():
      try:
        for upload_id in created_ids:
          upload = await session.execute(select(PublicUploadModel).where(PublicUploadModel.id == upload_id))
...
class TestPublicUploadServiceIntegration:
  ...
  async def test_upload_file_integration(
    self, integration_upload_service, db_session, test_integration_user, test_upload_file, setup_test_db_integration
  ):
    ...
    async for session in db_session:
      try:
        # Execute
        response = await integration_upload_service.post(file=test_upload_file, session=session, user=test_integration_user)
        ...
        # Verify in database
        query = select(PublicUploadModel).where(PublicUploadModel.filename == test_upload_file.filename)
    ...
    async for session in db_session:
      try:
        ...
        async def post_with_metadata(file, session, user):
          ...
          # Upload to S3
          key = f"uploads/{file.filename}"
          url = await integration_upload_service.s3_client.upload_file(file=BytesIO(file_content), key=key)
          ...
          # Store in database with metadata
          db_upload = PublicUploadModel(filename=file.filename, content_type=file.content_type, url=url, _metadata=metadata)
          session.add(db_upload)
          await session.commit()
          await session.refresh(db_upload)
        ...
        # Execute
        db_upload = await post_with_metadata(file=test_upload_file, session=session, user=test_integration_user)
    ...
    async for session in db_session:
      try:
        ...
        async def get_file_by_id(file_id, session):
          query = select(PublicUploadModel).where(PublicUploadModel.id == file_id)
          result = await session.execute(query)
    ...
    async for session in db_session:
      try:
        ...
        async def post_with_custom_path(file, session, user):
          ...
          url = await integration_upload_service.s3_client.upload_file(file=BytesIO(file_content), key=custom_key)
        ...
        # Execute
        db_upload = await post_with_custom_path(file=test_upload_file, session=session, user=test_integration_user)
  ...
  async def test_upload_delete_integration(
    ...
    async for session in db_session:
      try:
        # Upload a file first
        test_upload_file.file.seek(0)
        await integration_upload_service.post(file=test_upload_file, session=session, user=test_integration_user)
        ...
        query = select(PublicUploadModel).where(PublicUploadModel.id == upload_id)
        result = await session.execute(query)
    ...
    async for session in db_session:
      try:
        ...
        with open(temp_file_path, "rb") as file_obj:
          large_file = TestUploadFile(filename="test_integration_large.bin", file=file_obj, content_type="application/octet-stream")
  ...
  async def test_upload_unsupported_file_type(self, integration_upload_service, db_session, test_integration_user, setup_test_db_integration):
    ...
    async for session in db_session:
      try:
        ...
        # Create UploadFile
        with open(temp_file_path, "rb") as file_obj:
          js_file = TestUploadFile(filename="test_integration_script.js", file=file_obj, content_type="application/javascript")
