...
def mock_conversation():
  ...
  return MockConversationModel(id=uuid4(), organization_id=uuid4(), user_id=uuid4(), title="testing", description="", is_archived=False)


@pytest.fixture
def mock_chat_session():
  """Create a mock chat session."""
  return MockChatSessionModel(id=uuid4(), conversation_id=uuid4(), model_id=uuid4(), status="active", settings={"temperature": 0.7})


@pytest.fixture
def mock_message():
  """Create a mock message."""
  return MockMessageModel(id=uuid4(), chat_session_id=uuid4(), role="user", content="hello", token_used=5)


@pytest.fixture
def mock_model():
  """Create a mock LLM model."""
  return MockModelModel(id=uuid4(), name="gpt-4", provider="openai", version="4o")
...
def mock_conversation_service():
  ...
  async def mock_get_list(org_id, session, user):
    ...
    for i in range(5):
      conversation = MockConversationModel(
        organization_id=org_id, user_id=user["id"], title=f"Conversation {i + 1}", is_archived=False if i < 4 else True
      )
  ...
  async def mock_get_with_sessions(org_id, conversation_id, session, user):
    # Create mock data
    conversation = MockConversationModel(id=conversation_id, organization_id=org_id, user_id=user["id"], title="Test Conversation", is_archived=False)
    ...
    for i in range(3):
      ...
      chat_session = MockChatSessionModel(
        conversation_id=conversation_id, model_id=model_id, agent_id=agent_id, status="active" if i < 2 else "archived", settings={"temperature": 0.7}
      )
  ...
  async def mock_get_messages(org_id, conversation_id, limit, offset, session, user):
    ...
    return MockResponse(messages=message_list, total=total_count, has_more=(offset + limit < total_count))
  ...
  async def mock_stream_chat(org_id, chat_request: ChatRequestModel, session, user):
    ...
    chat_session = MockChatSessionModel(id=chat_session_id, conversation_id=uuid4(), model_id=uuid4())

    # Setup mock DB response
    session.execute.return_value.scalar_one_or_none.return_value = chat_session

    # Create user message
    user_message = MockMessageModel(chat_session_id=chat_session_id, role="user", content=chat_request.message)

    # Create assistant message
    assistant_message = MockMessageModel(
      chat_session_id=chat_session_id, role="assistant", content="This is a mock response from the assistant.", token_used=30
    )
    ...
    async def fake_stream():
      chunks = ["This ", "is ", "a ", "mock ", "response ", "from ", "the ", "assistant."]
      for chunk in chunks:
        yield {"text": chunk}
...
class TestConversationService:
  ...
  async def test_create_conversation(self, mock_conversation_service, mock_db_session, mock_user):
    ...
    # Call the service
    org_id = uuid4()
    conversation_data = MockResponse(title="New Conversation", description="Test description")

    response = await mock_conversation_service.post(org_id, conversation_data, session=mock_db_session, user=mock_user)
  ...
  async def test_update_conversation(self, mock_conversation_service, mock_db_session, mock_user, mock_conversation):
    ...
    # Call the service
    update_data = MockResponse(title="Updated Title", is_archived=True)

    response = await mock_conversation_service.put(conversation_id, update_data, session=mock_db_session, user=mock_user)
  ...
  async def test_update_conversation_not_found(self, mock_conversation_service, mock_db_session, mock_user):
    ...
    # Call the service
    update_data = MockResponse(title="Updated Title")

    # Verify exception is raised
    with pytest.raises(HTTPException) as excinfo:
      await mock_conversation_service.put(conversation_id, update_data, session=mock_db_session, user=mock_user)
  ...
  async def test_create_chat_session(self, mock_conversation_service, mock_db_session, mock_user, mock_conversation, mock_model):
    ...
    # Call the service
    session_data = MockResponse(conversation_id=conversation_id, model_id=model_id, settings={"temperature": 0.8})

    response = await mock_conversation_service.post_session(org_id, session_data, session=mock_db_session, user=mock_user)
  ...
  async def test_create_chat_session_conversation_not_found(self, mock_conversation_service, mock_db_session, mock_user):
    ...
    # Call the service
    session_data = MockResponse(conversation_id=conversation_id, model_id=model_id, settings={})

    # Verify exception is raised
    with pytest.raises(HTTPException) as excinfo:
      await mock_conversation_service.post_session(org_id, session_data, session=mock_db_session, user=mock_user)
  ...
  async def test_get_conversation_list(self, mock_conversation_service, mock_db_session, mock_user):
    ...
    response = await mock_conversation_service.get_list(org_id, session=mock_db_session, user=mock_user)
  ...
  async def test_get_conversation_with_sessions(self, mock_conversation_service, mock_db_session, mock_user):
    ...
    response = await mock_conversation_service.get(org_id, conversation_id, session=mock_db_session, user=mock_user)
  ...
  async def test_get_messages(self, mock_conversation_service, mock_db_session, mock_user):
    ...
    response = await mock_conversation_service.get_messages(
      org_id, conversation_id, limit=limit, offset=offset, session=mock_db_session, user=mock_user
    )
    ...
    assert response.has_more  # 5 < 15, so there are more messages
  ...
  async def test_stream_chat(self, mock_conversation_service, mock_db_session, mock_user, mock_chat_session):
    ...
    chat_request = ChatRequestModel(chat_session_id=chat_session_id, message="Hello, assistant!")

    stream = await mock_conversation_service.stream_chat(org_id, chat_request, session=mock_db_session, user=mock_user)
