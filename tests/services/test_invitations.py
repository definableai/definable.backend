...
def mock_role():
  """Create a mock role."""
  return MockRoleModel(name="Test Role", description="Test role description", organization_id=uuid4(), is_admin=False)
...
def mock_invitations_service():
  ...
  async def mock_get_list(org_id, params=None, session=None, user=None):
    ...
    # Calculate offset (unused in this mock but would be used in real DB query)
    # offset = (page - 1) * size
    ...
    for i in range(3):
      ...
      role = MockRoleModel(name=f"Role {i + 1}", description=f"Role {i + 1} Description", organization_id=org_id)
    ...
    return MockResponse(items=invitation_responses, total=3, page=page, size=size)

  async def mock_get(org_id, invitation_id, session=None, user=None):
    # Mock invitation with role info
    role = MockRoleModel(name="Test Role", description="Test Role Description", organization_id=org_id)
  ...
  async def mock_send(org_id, invitation_data, session=None, user=None):
    ...
    role = MockRoleModel(id=invitation_data.role_id, name="Test Role", description="Test Role Description", organization_id=org_id)
  ...
  async def mock_accept(action_request, session=None):
    ...
    return {
      ...
      "role": {"id": role_id, "name": "Test Role"},
    }
...
class TestInvitationService:
  ...
  async def test_get_list(self, mock_invitations_service, mock_db_session, mock_user):
    ...
    response = await mock_invitations_service.get_list(org_id, MockParams(), session=mock_db_session, user=mock_user)
  ...
  async def test_get_invitation(self, mock_invitations_service, mock_db_session, mock_user):
    ...
    response = await mock_invitations_service.get_get(org_id, invitation_id, session=mock_db_session, user=mock_user)
  ...
  async def test_send_invitation(self, mock_invitations_service, mock_db_session, mock_user, mock_role):
    ...
    # Call the service
    response = await mock_invitations_service.post_send(org_id, invitation_data, session=mock_db_session, user=mock_user)
  ...
  async def test_resend_invitation(self, mock_invitations_service, mock_db_session, mock_user):
    ...
    org_id = uuid4()
    invitation_id = uuid4()
    resend_request = MockResponse(invitation_id=invitation_id)

    # Call the service
    response = await mock_invitations_service.post_resend(org_id, resend_request, session=mock_db_session, user=mock_user)
  ...
  async def test_accept_invitation(self, mock_invitations_service, mock_db_session):
    ...
    action_request = MockResponse(token="test_token", email="invited@example.com")

    # Call the service
    response = await mock_invitations_service.post_accept(action_request, session=mock_db_session)
  ...
  async def test_reject_invitation(self, mock_invitations_service, mock_db_session):
    ...
    # Call the service
    response = await mock_invitations_service.post_reject(action_request, session=mock_db_session)
  ...
  async def test_send_invitation_with_existing(self, mock_invitations_service, mock_db_session, mock_user):
    ...
    # Create invitation data
    org_id = uuid4()
    invitation_data = MockResponse(role_id=uuid4(), invitee_email="existing@example.com", expiry_time=None)
    ...
    with pytest.raises(HTTPException) as excinfo:
      await mock_invitations_service.post_send(org_id, invitation_data, session=mock_db_session, user=mock_user)
  ...
  async def test_accept_non_pending_invitation(self, mock_invitations_service, mock_db_session):
    ...
    async def mock_accept_non_pending(action_request, session=None):
      # In a real implementation, we would first get the invitation by token/email
      # and then check its status. Here we mock that the invitation was found
      # but its status is not PENDING
      mock_db_session.execute.return_value.scalar_one_or_none.return_value = non_pending_invitation

      raise HTTPException(status_code=400, detail="Invitation is not in pending state")

    mock_invitations_service.post_accept = AsyncMock(side_effect=mock_accept_non_pending)

    # Call the service and expect exception
    with pytest.raises(HTTPException) as excinfo:
      await mock_invitations_service.post_accept(action_request, session=mock_db_session)
