...
class TestValidator:
  ...
  def test_config_parameter_validation_success(self):
    ...
    param = ConfigParameter(name="api_key", type="str", description="API key for service", required=True, default=None)
  ...
  def test_input_parameter_validation(self):
    ...
    param = InputParameter(name="location", type="str", description="Location to check", required=True, enum=["New York", "Tokyo", "London"])
  ...
  def test_tool_schema_success(self):
    ...
    schema = ToolSchema(
      ...
      configuration=[ConfigParameter(name="api_key", type="str", description="API key", required=True)],
      inputs=[InputParameter(name="location", type="str", description="Location", required=True)],
      output=OutputSpec(type="str", description="Weather information"),
      function_info=FunctionInfo(
        name="get_weather", is_async=True, description="Get weather info", code="async def get_weather(location): return f'Weather in {location}'"
      ),
  ...
  def test_tool_schema_validation_consistency(self):
    ...
    with pytest.raises(ValidationError) as excinfo:
      ToolSchema(
        ...
        inputs=[InputParameter(name="input", type="str", description="Input", required=True)],
        output=OutputSpec(type="str", description="Output"),
        function_info=FunctionInfo(name="test_func", description="Test function", code="def test_func(): pass"),
      )
  ...
  def test_validate_tool_json_success(self):
    ...
    valid_json = {
      ...
      "configuration": [{"name": "api_key", "type": "str", "description": "API key", "required": True}],
      "inputs": [{"name": "input", "type": "str", "description": "Input param", "required": True}],
      "output": {"type": "str", "description": "Output description"},
      "function_info": {"name": "test_func", "description": "Test function", "code": "def test_func(): pass"},
    }
