...
from src.libs.tools_factory.v1.testing_bp import generate_boilerplate, parse_configuration
...
class TestTestingBp:
  ...
  def test_generate_boilerplate_openai(self):
    ...
    config_items = [{"name": "api_key", "value": "weather_api_key", "type": "str"}]
  ...
  def test_generate_boilerplate_anthropic(self):
    ...
    boilerplate = generate_boilerplate(
      class_name=class_name, input_prompt=input_prompt, model_name=model_name, provider=provider, api_key=api_key, module_name=module_name
    )
