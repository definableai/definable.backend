...
class TestTranscription:
  ...
  def mock_os_operations(self):
    """Mock OS operations."""
    with (
      patch("src.libs.speech.v1.transcription.os.path.getsize") as mock_getsize,
      ...
      yield {"getsize": mock_getsize, "exists": mock_exists, "unlink": mock_unlink}
  ...
  async def test_transcribe_from_bytes(self, mock_settings, mock_temp_file, mock_open, mock_os_operations):
    ...
    with patch("src.libs.speech.v1.transcription.client.audio.transcriptions.create", new_callable=AsyncMock) as mock_create:
  ...
  async def test_transcribe_invalid_source(self):
    ...
    # Execute and Assert
    with pytest.raises(ValueError) as excinfo:
      await transcribe(source="not_a_url")  # Not a valid URL, still a string

    assert "Source must be either a URL or bytes data" in str(excinfo.value)

  @pytest.mark.asyncio
  async def test_transcribe_url_content_type_inference(
    self, mock_settings, mock_httpx_client, mock_temp_file, mock_open, mock_os_operations, mock_mimetypes
  ):
  ...
  async def test_transcribe_with_language_parameter(
    self, mock_settings, mock_temp_file, mock_httpx_client, mock_os_operations, mock_mimetypes, mock_open
  ):
