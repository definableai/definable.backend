...
class TestS3Client:
  ...
  async def test_upload_file_uploadfile(self, s3_client, test_upload_file):
    ...
    with patch.object(s3_client, "_get_client", return_value=mock_client):
      # Execute
      result = await s3_client.upload_file(file=test_upload_file, key="uploads/test_file.txt", content_type="text/plain")
  ...
  async def test_upload_file_bytesio(self, s3_client, test_file_bytesio):
    ...
    with patch.object(s3_client, "_get_client", return_value=mock_client):
      # Execute
      result = await s3_client.upload_file(file=test_file_bytesio, key="uploads/test_file.txt")
  ...
  async def test_upload_file_bytes(self, s3_client, test_file_bytes):
    ...
    with patch.object(s3_client, "_get_client", return_value=mock_client):
      # Execute
      result = await s3_client.upload_file(file=test_file_bytes, key="uploads/test_file.txt")
  ...
  async def test_upload_file_error(self, s3_client):
    ...
    with patch.object(s3_client, "_get_client", return_value=mock_client):
      ...
      with pytest.raises(HTTPException) as excinfo:
        await s3_client.upload_file(file=b"test", key="test.txt")
  ...
  async def test_get_presigned_url(self, s3_client):
    ...
    with patch.object(s3_client, "_get_client", return_value=mock_client):
      # Execute
      result = await s3_client.get_presigned_url(key="test/presigned.txt", expires_in=3600, operation="get_object")
...
class TestS3ClientErrorHandling:
  ...
  async def test_missing_credentials(self):
    ...
    with patch("src.libs.s3.v1.basic_ops.settings") as mock_settings:
      ...
      with pytest.raises(Exception):
        await client.upload_file(file=BytesIO(b"test"), key="test.txt")
  ...
  async def test_nonexistent_bucket(self, s3_client):
    ...
    with patch.object(s3_client, "_get_client", return_value=error_client):
      ...
      with pytest.raises(HTTPException) as excinfo:
        await s3_client.upload_file(file=BytesIO(b"test"), key="test.txt")
