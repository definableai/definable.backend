...
class TestFirecrawlEdgeCases:
  ...
  def test_scrape_url_empty_content(self, mock_scrape_url):
    ...
    # Setup mock
    mock_scrape_url.return_value = {"markdown": "", "metadata": {"statusCode": 200}}
  ...
  def test_scrape_url_server_error(self, mock_scrape_url):
    ...
    # Setup mock
    mock_scrape_url.return_value = {"markdown": "", "metadata": {"statusCode": 500}}
  ...
  async def test_crawl_http_403(self, mock_post):
    ...
    # Setup mock
    mock_response = MagicMock()
    mock_response.raise_for_status.side_effect = httpx.HTTPStatusError("403 Forbidden", request=MagicMock(), response=MagicMock(status_code=403))
    mock_post.return_value = mock_response
