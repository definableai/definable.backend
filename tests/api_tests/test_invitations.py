...
class TestInvitationsAPI:
  ...
  async def test_list_invitations(self, client, mock_db_session, auth_headers, org_id):
    ...
    with patch.object(client, "get", autospec=True) as mock_get:
      ...
      response = client.get(f"/api/invitations?org_id={org_id}&page=1&size=10", headers=auth_headers)
  ...
  async def test_get_invitation(self, client, mock_db_session, auth_headers, org_id):
    ...
    with patch.object(client, "get", autospec=True) as mock_get:
      ...
      # Make the API request
      response = client.get(f"/api/invitations/{invitation_id}?org_id={org_id}", headers=auth_headers)
  ...
  async def test_send_invitation(self, client, mock_db_session, auth_headers, org_id):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      # Make the API request
      response = client.post(f"/api/invitations/send?org_id={org_id}", headers=auth_headers, json=invitation_data)
  ...
  async def test_resend_invitation(self, client, mock_db_session, auth_headers, org_id):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      # Make the API request
      response = client.post(f"/api/invitations/resend?org_id={org_id}", headers=auth_headers, json=resend_data)
  ...
  async def test_accept_invitation(self, client, mock_db_session):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      org_id = str(uuid4())
      role_id = str(uuid4())
      action_data = {"token": "test_token", "email": "invite@example.com"}
      ...
      response = client.post("/api/invitations/accept", json=action_data)
  ...
  async def test_reject_invitation(self, client, mock_db_session):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      mock_response = MagicMock()
      mock_response.status_code = 200
      mock_response.json.return_value = {"message": "Invitation rejected successfully"}
      mock_post.return_value = mock_response

      # Make the API request - no auth headers needed for this endpoint
      response = client.post("/api/invitations/reject", json=action_data)
