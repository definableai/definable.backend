...
class TestAgentsAPI:
  ...
  async def test_list_org_specific_agents(self, client, mock_db_session, auth_headers, org_id):
    ...
    with patch.object(client, "get", autospec=True) as mock_get:
      ...
      response = client.get(f"/api/agents/list?org_id={org_id}", headers=auth_headers)
  ...
  async def test_list_all_agents(self, client, mock_db_session, auth_headers):
    ...
    with patch.object(client, "get", autospec=True) as mock_get:
      ...
      # Make the API request
      response = client.get("/api/agents/list_all", headers=auth_headers)
  ...
  async def test_execute_agent(self, client, mock_db_session, auth_headers, org_id):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      # Create test data
      agent_id = str(uuid4())
      execution_data = {"input": "What's the weather like today?", "parameters": {"location": "New York"}}
      ...
      # Make the API request
      response = client.post(f"/api/agents/execute?org_id={org_id}&agent_id={agent_id}", headers=auth_headers, json=execution_data)
