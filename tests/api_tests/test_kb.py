...
def test_kb_data():
  ...
  return {
    "name": "Test KB",
    "settings": {"embedding_model": "text-embedding-3-large", "max_chunk_size": 500, "chunk_overlap": 50, "separator": "\n\n"},
  }
...
def test_search_data():
  ...
  return {"query": "test query", "limit": 3, "score_threshold": 0.0}
...
class TestKnowledgeBaseAPI:
  ...
  async def test_create_kb(self, client, mock_db_session, auth_headers, test_kb_data):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      response = client.post("/api/kb/create?org_id=test-org-id", headers=auth_headers, json=test_kb_data)
  ...
  async def test_list_kb(self, client, mock_db_session, auth_headers):
    ...
    with patch.object(client, "get", autospec=True) as mock_get:
      ...
      # Make the API request
      response = client.get("/api/kb/list?org_id=test-org-id", headers=auth_headers)
  ...
  async def test_search_chunks(self, client, mock_db_session, auth_headers, test_search_data):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      mock_response.status_code = 200
      mock_response.json.return_value = [
        {"chunk_id": chunk_id, "document_id": document_id, "content": "Test content matching the query", "metadata": {"page": 1}, "score": 0.95}
      ]
      mock_post.return_value = mock_response

      # Make the API request
      response = client.post("/api/kb/search_chunks?org_id=test-org-id&kb_id=test-kb-id", headers=auth_headers, json=test_search_data)
