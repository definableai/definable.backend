...
class TestOrganizationAPI:
  ...
  async def test_list_orgs(self, client, mock_db_session, auth_headers):
    ...
    with patch.object(client, "get", autospec=True) as mock_get:
      ...
      mock_response.json.return_value = [
        {"id": org_id, "name": "Default Org", "slug": "default-org-12345678", "settings": {}},
        {"id": str(uuid4()), "name": "Test Org 2", "slug": "test-org-2-12345678", "settings": {}},
      ]
      mock_get.return_value = mock_response

      # Make the API request
      response = client.get("/api/org/list", headers=auth_headers)
  ...
  async def test_create_org(self, client, mock_db_session, auth_headers, test_org_data):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      # Make the API request
      response = client.post(f"/api/org/create_org?name={test_org_data['name']}", headers=auth_headers)
