...
def test_conversation_data():
  ...
  return {"title": "Test Conversation", "is_archived": False}


@pytest.fixture
def test_chat_session_data():
  """Create test chat session data."""
  return {"conversation_id": str(uuid4()), "model_id": str(uuid4())}


@pytest.fixture
def test_message_data():
  """Create test message data."""
  return {"message": "Hello, this is a test message", "chat_session_id": str(uuid4())}
...
class TestConversationAPI:
  ...
  async def test_create_conversation(self, client, mock_db_session, auth_headers, test_conversation_data, org_id):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      response = client.post(f"/api/conversation/create?org_id={org_id}", headers=auth_headers, json=test_conversation_data)
  ...
  async def test_list_conversations(self, client, mock_db_session, auth_headers, org_id):
    ...
    with patch.object(client, "get", autospec=True) as mock_get:
      ...
      # Make the API request
      response = client.get(f"/api/conversation/list?org_id={org_id}", headers=auth_headers)
  ...
  async def test_create_chat_session(self, client, mock_db_session, auth_headers, test_chat_session_data, org_id):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      # Make the API request
      response = client.post(f"/api/conversation/create_session?org_id={org_id}", headers=auth_headers, json=test_chat_session_data)
  ...
  async def test_stream_chat(self, client, mock_db_session, auth_headers, test_message_data, org_id):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      # Make the API request
      response = client.post(f"/api/conversation/stream_chat?org_id={org_id}", headers=auth_headers, json=test_message_data)
  ...
  async def test_get_messages(self, client, mock_db_session, auth_headers, org_id):
    ...
    with patch.object(client, "get", autospec=True) as mock_get:
      ...
      # Make the API request
      response = client.get(f"/api/conversation/messages?org_id={org_id}&conversation_id={conversation_id}&limit=10&offset=0", headers=auth_headers)
