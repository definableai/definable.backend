...
def test_llm_model_data():
  ...
  return {"name": "Test Model", "provider": "openai", "version": "4o", "is_active": True, "config": {}}
...
class TestLLMAPI:
  ...
  async def test_add_model(self, client, mock_db_session, auth_headers, test_llm_model_data):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      response = client.post("/api/llm/add", headers=auth_headers, json=test_llm_model_data)
  ...
  async def test_list_models(self, client, mock_db_session, auth_headers):
    ...
    with patch.object(client, "get", autospec=True) as mock_get:
      ...
      mock_response.status_code = 200
      mock_response.json.return_value = [
        {"id": model_id, "name": "Test Model", "provider": "openai", "version": "4o", "is_active": True, "config": {}}
      ]
      mock_get.return_value = mock_response

      # Make the API request
      response = client.get("/api/llm/list?org_id=test-org-id", headers=auth_headers)
