...
def test_role_data():
  ...
  return {"name": "Test Role", "description": "Test role description", "hierarchy_level": 10, "permission_ids": [str(uuid4())]}
...
class TestRolesAPI:
  ...
  async def test_create_role(self, client, mock_db_session, auth_headers, test_role_data):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      response = client.post(f"/api/roles/create?org_id={org_id}", headers=auth_headers, json=test_role_data)
  ...
  async def test_list_permissions(self, client, mock_db_session, auth_headers):
    ...
    with patch.object(client, "get", autospec=True) as mock_get:
      ...
      mock_response.status_code = 200
      mock_response.json.return_value = [
        {"id": permission_id, "name": "View Organizations", "resource": "organization", "action": "view"},
        {"id": str(uuid4()), "name": "Create Organizations", "resource": "organization", "action": "create"},
      ]
      mock_get.return_value = mock_response

      # Make the API request
      response = client.get("/api/roles/list_permissions", headers=auth_headers)
