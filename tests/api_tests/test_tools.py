...
def test_category_data():
  ...
  return {"name": "Test Category", "description": "Test category description"}
...
def test_tool_data():
  ...
  return {
    ...
    "logo_url": "",
    "is_public": True,
    "configuration": [{"name": "api_key", "type": "str", "description": "API key for the service", "required": True, "default": None}],
    "inputs": [{"name": "query", "type": "str", "description": "The query to process", "required": True, "default": None}],
    "outputs": {"type": "str", "description": "The result of the tool execution"},
    "settings": {
      "function_info": {
        "name": "Test Function",
        "is_async": True,
      ...
      "deployment": {"framework": "agno", "toolkit_class": True, "standalone_function": False},
    },
  }
...
class TestToolsAPI:
  ...
  async def test_create_category(self, client, mock_db_session, auth_headers, test_category_data, org_id):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      response = client.post(f"/api/tools/create_category?org_id={org_id}", headers=auth_headers, json=test_category_data)
  ...
  async def test_list_categories(self, client, mock_db_session, auth_headers):
    ...
    with patch.object(client, "get", autospec=True) as mock_get:
      ...
      mock_response.status_code = 200
      mock_response.json.return_value = [
        {"id": category_id, "name": "Test Category", "description": "Test category description", "organization_id": str(uuid4())}
      ]
      mock_get.return_value = mock_response

      # Make the API request
      response = client.get("/api/tools/list_categories", headers=auth_headers)
  ...
  async def test_create_tool(self, client, mock_db_session, auth_headers, test_tool_data, org_id):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      # Make the API request
      response = client.post(f"/api/tools?org_id={org_id}", headers=auth_headers, json=test_tool_data)
  ...
  async def test_list_all_tools(self, client, mock_db_session, auth_headers):
    ...
    with patch.object(client, "get", autospec=True) as mock_get:
      ...
      # Make the API request
      response = client.get("/api/tools/list_all", headers=auth_headers)
  ...
  async def test_test_tool(self, client, mock_db_session, auth_headers):
    ...
    with patch.object(client, "post", autospec=True) as mock_post:
      ...
      test_request_data = {
        ...
        "config_items": [{"name": "api_key", "value": "test_api_key"}],
        "provider": "anthropic",
      ...
      mock_response = MagicMock()
      mock_response.status_code = 200
      mock_response.json.return_value = {"result": "Successfully processed query: test query", "time_taken": 1.23}
      mock_post.return_value = mock_response

      # Make the API request
      response = client.post(f"/api/tools/test_tool?tool_id={tool_id}", headers=auth_headers, json=test_request_data)
