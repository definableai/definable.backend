      ...
      if exists:
        ...
        await db.execute(
          text(f"""
            ...
            (name, amount, unit, measure, service, action, description, created_at, updated_at)
            VALUES
            (:name, :amount, :unit, :measure, :service, :action, :description, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
          """),
          charge,
        )
