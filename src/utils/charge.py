...
class Charge:
  ...
  def calculate_and_update_with_session(self, content=None, metadata=None, status="completed", additional_metadata=None, job_id=None):
    """Sync version of calculate_and_update with job tracking."""
    self.logger.info(f"Starting sync calculate_and_update for charge: {self.name}")

    try:
      # Get charge details
      charge_details = self._get_charge_details_with_session(self.name, self.session)
      ...
      if job_id:
        ...
        meta["job_status"] = "completed" if status == "completed" else "processing"

      # Calculate based on charge measure type
      measure_qty = 1
      if charge_details.measure == "page" and metadata and "pages" in metadata:
        measure_qty = metadata.get("pages", 1)
        meta["page_count"] = measure_qty
        self.logger.info(f"Page count calculated: {measure_qty}")
      elif charge_details.measure == "file":
        measure_qty = 1  # Fixed per file
        meta["file_count"] = 1
      ...
      # Update transaction with calculated amount
      transaction = self._get_transaction_with_session_internal(for_update=True)
      current_metadata = transaction.transaction_metadata or {}
      current_qty = int(current_metadata.get("qty", 1))
      qty_increment = max(0, measure_qty - current_qty)
      ...
      if qty_increment > 0:
        ...
        charge_details = self._get_charge_details_with_session(self.name, self.session)
        additional_amount = charge_details.amount * qty_increment
        
        wallet = self._get_wallet_with_session_internal(self.org_id, self.session, for_update=True)
        
        # Update transaction
        original_amount = transaction.credits
        transaction.credits = original_amount + additional_amount
        current_metadata["qty"] = current_qty + qty_increment
        transaction.transaction_metadata = {**current_metadata, **meta}
      ...
      # Convert to DEBIT if completed
      if status == "completed":
        self._finalize_with_session(meta)
  ...
  def delete_transaction_with_session(self, reason: Optional[str] = None, job_id: Optional[str] = None):
    """Sync version of delete to cancel charge."""
    transaction = self._get_transaction_with_session_internal(for_update=True)
    ...
    transaction.type = TransactionType.RELEASE
    transaction.status = TransactionStatus.COMPLETED
    transaction.description = f"Released hold for {self.name}" + (f": {reason}" if reason else "")

    # Add job tracking to metadata
    if transaction.transaction_metadata:
      if reason:
        transaction.transaction_metadata["release_reason"] = reason
      if job_id:
        transaction.transaction_metadata["job_id"] = str(job_id)
        transaction.transaction_metadata["job_status"] = "failed"
      transaction.transaction_metadata["original_status"] = "CANCELLED"
  ...
  def _get_charge_details_with_session(self, name: str, session):
    from sqlalchemy import select
  ...
  def _get_transaction_with_session_internal(self, for_update: bool = False):
    from sqlalchemy.sql import text
    from sqlalchemy import select
    from models import TransactionModel
    ...
    if for_update:
      lock_query = text("SELECT * FROM transactions WHERE id = :tx_id FOR UPDATE")
      self.session.execute(lock_query, {"tx_id": str(self.transaction_id)})
        
    query = select(TransactionModel).where(TransactionModel.id == self.transaction_id)
    result = self.session.execute(query)
    return result.scalar_one_or_none()

  def _get_wallet_with_session_internal(self, org_id: UUID, session, for_update: bool = False):
    from sqlalchemy.sql import text
    from sqlalchemy import select
    ...
    if for_update:
      lock_query = text("SELECT * FROM wallets WHERE organization_id = :organization_id FOR UPDATE")
      session.execute(lock_query, {"organization_id": str(org_id)})
  ...
  def _finalize_with_session(self, additional_metadata):
