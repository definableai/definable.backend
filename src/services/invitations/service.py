...
from .model import InvitationModel
from .schema import (
    InvitationCreate,
    InvitationResponse,
...
class InvitationService:
    ...
    async def post_send(
        ...
        await self._validate_inviter_role(
            user["id"], 
            org_id,
    ...
    async def _validate_inviter_role(
        ...
        query = select(self.models["OrganizationMemberModel"]).where(
            and_(
                self.models["OrganizationMemberModel"].user_id == user_id,
                self.models["OrganizationMemberModel"].organization_id == org_id,
                self.models["OrganizationMemberModel"].status == "active",
            )
        )
        result = await session.execute(query)
        ...
        role_query = select(self.models["RoleModel"]).where(
            and_(
                self.models["RoleModel"].id == role_id,
                self.models["RoleModel"].organization_id == org_id,
            )
        )
        ...
        # Get inviter's role
        inviter_role_query = select(self.models["RoleModel"]).where(
            self.models["RoleModel"].id == member.role_id
        )
    ...
    async def _send_invitation_email(self, invitation: InvitationModel, org_id: UUID, session: AsyncSession) -> None:
        try:
            org = await self.models["OrganizationModel"].read(org_id)
            if not org:
