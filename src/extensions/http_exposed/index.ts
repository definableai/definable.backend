...
let getEndpointDecoration: vscode.TextEditorDecorationType;
let postEndpointDecoration: vscode.TextEditorDecorationType;
let putEndpointDecoration: vscode.TextEditorDecorationType;
let deleteEndpointDecoration: vscode.TextEditorDecorationType;
let activeEditor: vscode.TextEditor | undefined;
let decorationsTimeout: NodeJS.Timeout | undefined;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    deleteEndpointDecoration = vscode.window.createTextEditorDecorationType({
        color: '#F14C4C', // Bright red for DELETE (changed from orange)
        fontStyle: 'italic',
        textDecoration: 'none',
...
function updateDecorations() {
    ...
    decorationsTimeout = setTimeout(() => {
        try {
            ...
            const getDecorations: vscode.DecorationOptions[] = [];
            const postDecorations: vscode.DecorationOptions[] = [];
            const putDecorations: vscode.DecorationOptions[] = [];
            ...
            const httpExposedRanges: { start: number; end: number }[] = [];
                ...
                if (isInHttpExposed) {
                    ...
                    const decoration = {
                        ...
                        hoverMessage: new vscode.MarkdownString(
                            `**${httpMethod.toUpperCase()} Endpoint**: \`${endpointName}\`\n\n` +
...
export function deactivate() {
    ...
    if (getEndpointDecoration) getEndpointDecoration.dispose();
    if (postEndpointDecoration) postEndpointDecoration.dispose();
    if (putEndpointDecoration) putEndpointDecoration.dispose();
    if (deleteEndpointDecoration) deleteEndpointDecoration.dispose();
