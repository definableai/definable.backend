...
class BasePDFReader(Reader):
  def __init__(self, **kwargs):
    ...
    # Ensure separators is initialized from the dataclass field default
    if not hasattr(self, "separators") or self.separators is None:
...
class PDFReader(BasePDFReader):
  ...
  async def async_read(self, pdf: Union[str, Path, IO[Any]]) -> List[Document]:
    ...
    documents = await asyncio.gather(*[_process_document(doc_name, page_number, page) for page_number, page in enumerate(doc_reader.pages, start=1)])
